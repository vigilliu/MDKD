...
from distiller_zoo import DistillKL, HintLoss, CAMKD, DKD
...
def main_worker(gpu, ngpus_per_node, opt):
    ...
    if opt.distill == 'kd':
        ...
        criterion_kd = CAMKD()#创建CAMKD类实例
        feat_t_list = [feat_t[-2] for feat_t in feat_t_list]#选择教师倒数第二个特征图
        cal_weight = CalWeight(feat_s[-2], feat_t_list, opt)#它接收学生模型的倒数第二个特征图(feat_s[-2])、教师模型的倒数第二个特征图列表(feat_t_list)以及opt对象作为输入
    ...
    #自己加的
    criterion_list.append(criterion_dkd)
