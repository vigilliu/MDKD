...
"""def target_class(logits_teacher,target,temperature):
    gt_mask = _get_gt_mask(logits_teacher, target)
    pred_teacher = F.softmax(logits_teacher / temperature, dim=1)
    t1 = (pred_teacher* gt_mask).sum(dim=1, keepdims=True)"""

def target_class(logits_teacher, target, temperature):
    gt_mask = _get_gt_mask(logits_teacher, target)
    pred_teacher = F.softmax(logits_teacher / temperature, dim=1)
    target_logits = torch.masked_select(pred_teacher, gt_mask)
...
def train_distill_multi_teacher(epoch, train_loader, module_list, criterion_list, optimizer, opt):
    ...
    # set modules as train()把所有模型设为训练模式
    for module in module_list:
        module.train()
    # set teacher as eval()
    #[model_t.eval() for model_t in module_list[-opt.teacher_num:]]

    criterion_cls = criterion_list[0]#获取交叉熵
    criterion_div = criterion_list[1]#获取kl散度
    criterion_kd = criterion_list[2]#CAMKD
    criterion_dkd = criterion_list[3]#dkd

    model_s = module_list[0]#学生模型是第一个

    # model_t = module_list[-1]
    model_t_list = module_list[-opt.teacher_num:]#把后面的教师模型加进列表
    ...
    for idx, data in enumerate(train_loader):
        ...
        # ===================forward=====================
        feat_s, logit_s = model_s(input, is_feat=True, preact=opt.preact)#通过学生模型对输入数据进行前向传播，获取学生模型特征和输出
        feat_t_list = []
        logit_t_list = []
        feat_t2_list = []
        logit_t2_list = []
        ...
        for model_t in model_t_list:
            ...
            feat_t2, logit_t2 = model_t(input, is_feat=True, preact=opt.preact)
            feat_t2 = [f.detach() for f in feat_t2]
            feat_t2_list.append(feat_t2)
            logit_t2_list.append(logit_t2)
        ...
        if opt.ensemble_method == "AVERAGE_LOSS":
            ...
            """这段是tcweight的笑容试验
            loss_t_list = [target_class(logit_t, target, temperature=4) for logit_t in logit_t_list]
            loss_t = torch.stack(loss_t_list, dim=0)
            attention = (F.softmax(loss_t, dim=0)) / (opt.teacher_num - 1)
            loss_div_list = [criterion_dkd(logit_s, logit_t, target)
                             for logit_t in logit_t_list]
            ...
            #1 criterion_cls_lc = nn.CrossEntropyLoss(reduction='none')
            #1 loss_t_list = [criterion_cls_lc(logit_t, target) for logit_t in logit_t_list]
            #1 print(loss_t_list,'loss_t_list')
            for logit_t, logit_t2 in zip(logit_t_list, logit_t2_list):
                kl_divergence = torch.nn.functional.kl_div(logit_t, logit_t2, reduction='batchmean')
                #_, kl_divergence = dkd_loss(logit_t, logit_t2, target, alpha=1, beta=8, temperature=4)
                kl_divergence_list.append(kl_divergence)
            #1 print(mse_list,'mse_list')
            '''1
            for logit_t, logit_t2 in zip(logit_t_list, logit_t2_list):
                mse_loss = F.mse_loss(logit_t, logit_t2, reduction='none')  # 不进行降维操作
                mse_loss = mse_loss.view(logit_t.size(0), -1)  # 调整形状为 (64, 100)
                mse_list.append(mse_loss)'''
            
            loss_t = torch.stack(kl_divergence_list, dim=0)
