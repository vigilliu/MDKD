...
class VGG(nn.Module):
    ...
    def __init__(self, cfg, batch_norm=False, num_classes=1000, dropout_rate=0.2):
    ...
    def distill_seq(self):
        ...
        #    self.block0, self.relu))       
        feat_m.append(nn.Sequential(
            self.pool0, self.block1, self.relu))
        feat_m.append(nn.Sequential(
            self.pool1, self.block2, self.relu))
        feat_m.append(nn.Sequential(
            self.pool2, self.block3, self.relu))
        feat_m.append(nn.Sequential(
            self.block4, self.relu))
        feat_m.append(nn.Sequential(
            self.pool4,
