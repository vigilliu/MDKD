...
class CalWeight(nn.Module):
    def __init__(self, feat_s, feat_t_list, opt):
        ...
        s_channel = feat_s.shape[1]#获取学生模型特征图的通道数
        for i in range(len(feat_t_list)):#对于教师模型特征图列表中的每个特征图，执行以下操作
    ...
    def forward(self, feat_s, feat_t_list, model_t_list=None):
        ...
        trans_feat_s_list = []#转换后的学生特征图
        output_feat_t_list = []#和教师模型的输出特征图
        s_H = feat_s.shape[2]
